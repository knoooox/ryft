...
const ProfileModule = {
    ...
    async toggleEditMode() {
        ...
        if (this.isEditingProfile) {
            ...
            if (!nameVal || nameVal.trim() === '') { this.tg?.showAlert("Пожалуйста, введите Фамилию и Имя"); this.isEditingProfile = true; return; }
            if (dobVal && !this.validateDOB(dobVal)) { this.tg?.showAlert("Некорректная дата рождения. Формат: ДД.ММ.ГГГГ"); this.isEditingProfile = true; return; }
    ...
    async openMyAthletes() {
        ...
        try {
            ...
            container.innerHTML = `
                <div style="display: flex; justify-content: space-between; align-items: center; padding: 10px 5px 20px; padding-top: var(--search-dynamic-top, 60px);">
                    <div style="font-size: 14px; font-weight: 800; color: #fff; letter-spacing: 0.5px;">МОИ СПОРТСМЕНЫ (${data.athletes.length})</div>
                    <button class="search-cancel" onclick="app.closeSearch()" style="margin: 0; padding: 0;">Назад</button>
                </div>
            `;
    ...
    openShareModal() {
        try {
            ...
            this.setText('share-link-text', `t.me/RyftAppBot/RYFT`);
            ...
            if (copyBtn) {
                ...
                copyBtn.innerHTML = `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2.5"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"></rect><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path></svg>`;
            }
