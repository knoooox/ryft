...
const app = {
    ...
    async init() {
        ...
        if (this.tg) {
            ...
            const calculateSafeOffsets = () => {
                if ((window.innerHeight / window.screen.height) > 0.8) wasFullScreen = true;
                let topOffset = '16px';
                if (wasFullScreen) {
                    if (this.tg.platform === 'ios') topOffset = 'calc(env(safe-area-inset-top) + 54px)';
                    else if (this.tg.platform === 'android') topOffset = '78px';
                    else topOffset = '65px';
                } else topOffset = this.tg.platform === 'ios' ? 'calc(env(safe-area-inset-top) + 12px)' : '24px';
                document.documentElement.style.setProperty('--search-dynamic-top', topOffset);
            };
            calculateSafeOffsets();
            window.addEventListener('resize', () => requestAnimationFrame(calculateSafeOffsets));
            setTimeout(calculateSafeOffsets, 250);
        }
        ...
        if (currentUser && currentUser.id === 861703506) {
    ...
    async fetchLocations() {
        try {
            ...
            if(res.ok) this.locationsData = await res.json();
        } catch(e) {}
    },
