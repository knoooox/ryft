...
# Локальное хранение отключено. Используем сервера Telegram.
...
def admin_scan():
    ...
    for uid in users:
        try:
            res = requests.post(url, json={'chat_id': uid, 'action': 'typing'}, timeout=2)
...
def admin_search_users():
    ...
    try:
        ...
        base_query = '''
            SELECT u.user_id, u.full_user_name as full_name, u.username, 
...
def get_profile(user_id):
    ...
    return jsonify({
        ...
        'photo_id': p.get('photo_file_id'),  # Теперь берем из profile_cards (p), а не из users (u)!
...
# ==========================================
# 5. РАБОТА С ФОТОГРАФИЯМИ И ПОИСК (ПОЛНОСТЬЮ ЛОКАЛЬНО)
# ==========================================
...
def get_photo(file_id):
    ...
    try:
        ...
        # 2. Скачиваем картинку в оперативную память
        ...
        # 3. Отдаем байты картинки прямо в браузер
        return send_file(
...
def upload_photo():
    ...
    # Если прислали пустую строку — значит пользователь удаляет аватарку
    ...
    try:
        # Убираем префикс base64, если он есть (data:image/jpeg;base64,...)
        if "," in image_data_base64:
            image_data_base64 = image_data_base64.split(",")[1]

        # Декодируем картинку из браузера в сырые байты
        img_data = base64.b64decode(image_data_base64)

        # Отправляем фото на сервера Телеграма
        tg_url = f"https://api.telegram.org/bot{BOT_TOKEN}/sendPhoto"
        files = {'photo': ('avatar.jpg', img_data, 'image/jpeg')}

        # Отправляем фото тебе (ADMIN_ID) без звука.
        # Это нужно только для того, чтобы Телеграм сгенерировал нам file_id
        payload = {'chat_id': ADMIN_ID, 'disable_notification': True}

        r = requests.post(tg_url, data=payload, files=files).json()

        if not r.get('ok'):
            raise Exception("Ошибка загрузки на сервера Telegram")

        # В ответе приходит массив photo. Последний элемент — это фото в максимальном качестве
        new_file_id = r['result']['photo'][-1]['file_id']

        # Записываем телеграмовский file_id в БД
        cursor.execute('SELECT user_id FROM profile_cards WHERE user_id = ?', (user_id,))
        if cursor.fetchone():
        ...
        # Возвращаем новый file_id на фронтенд, чтобы он моментально обновил картинку
...
def search_users():
    ...
    try:
        ...
        # Забираем photo_filename из users, и переименовываем его в photo_file_id для фронта
        if query.isdigit():
            cursor.execute('''
...
def check_sub(internal_id):
    ...
    try:
        cur = conn.cursor()
        # Ищем реальный TG ID по нашему внутреннему ID базы
        cur.execute("SELECT tg_id FROM users WHERE user_id = ?", (internal_id,))
        row = cur.fetchone()

        if not row or not row['tg_id']:
            return jsonify({'subscribed': True})  # Не блокируем, если нет данных

        tg_id = row['tg_id']  # ВОТ ОН, реальный ID для Телеграма
        ...
        return jsonify({'subscribed': True, 'error': str(e)})
    finally:
        conn.close()
...
def redirect_short_link(short_link):
    ...
    try:
        ...
        if user:
            # Теперь редирект не на telegram webapp, а на твой сайт
